    ...
    public class SqlServerTableBuildParser : ParserBase
    {
        ...
        public override string Parsing(ref List<IDbDataParameter> DbParameters)
        {
            ...
            if (fkBuffers.Length > 0)
                buffers.Append(",").AppendLine().AppendFormat(fkBuffers.ToString());
            buffers.AppendLine();
            buffers.Append(");");
        ...
        private void ParseForeignKey(string table, DbTableColumnDefinition definition, StringBuilder writer)
        {
            ...
            writer.AppendFormat("\tCONSTRAINT FK_{0}_{1} FOREIGN KEY ({2}{1}{3})", table, definition.Name, ElemIdentifierL, ElemIdentifierR);
